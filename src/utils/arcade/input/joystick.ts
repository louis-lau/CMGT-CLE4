...
export class Joystick {
    ...
    // Joystick identifier
    public get JoystickNumber(): number {
    ...
    public set Gamepad(gamepad: Gamepad) {
        this.gamepad = gamepad
    }

    // previous gamepad
    public get PreviousGamepad(): Gamepad {
        return this.previousGamepad
    }
    public set PreviousGamepad(previousGamepad: Gamepad) {
    ...
    private readGamepad(gamepad: Gamepad): void {
        for (let index = 0; index < this.numberOfBUttons; index++) {
            // if(this.buttonPressed(gamepad.buttons[index])) console.log(gamepad.buttons[index]);
